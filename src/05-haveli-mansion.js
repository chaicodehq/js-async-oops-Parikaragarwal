...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    if (this.#residents.find((resident) =>
      resident.name === name
    )) {
      return { success: false, message: "Already a resident!" };
    }
    
    const newResident = { name, role, addedAt: new Date().toISOString() };
  ...
  removeResident(name, passcode) {
    ...
    this.#residents[this.#residents.length]]
      =
    [this.#residents[this.#residents.length],
        this.#residents[residentIndex]];
  ...
  verifyAccess(name) {
    ...
    if (resident) {
      this.#accessLog.push(
        { name, time: new Date().toISOString(), allowed: true }
      );
