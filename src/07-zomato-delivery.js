...
export function placeOrder(restaurant, items) {
  return new Promise((res, rej) => {
    if (!restaurant || !Array.isArray(items)
...
export function confirmOrder(order) {
  return new Promise((res, rej) => {
    if (!Object.hasOwn(order, 'orderId') ||
      order.status!=='placed')
    {
...
export function processMultipleOrders(orderList) {
  const responses = orderList.map( order => {
  ...
  //if commented code is used we will use: Promise.all(responses);
}
